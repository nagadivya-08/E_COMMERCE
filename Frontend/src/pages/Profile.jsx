    ...
    return (
        <div className="profile-page">
            ...
            <div style={{ padding: "80px 20px 40px", maxWidth: "900px", margin: "0 auto" }}>
                {/* User Card */}
                <div className="user-card" style={{ display: "flex", alignItems: "center", gap: "20px", padding: "20px", backgroundColor: "#f0f4f8", borderRadius: "10px", marginBottom: "30px" }}>
                    <div style={{ width: "80px", height: "80px", borderRadius: "50%", backgroundColor: "#374151", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem" }}>👤</div>
                    <div>
                        <h2 style={{ margin: 0 }}>John Doe</h2>
                        <p style={{ margin: "5px 0", color: "#9ca3af" }}>john.doe@example.com</p>
                    </div>
                </div>

                {/* Tabs Navigation */}
                <div style={{ display: "flex", borderBottom: "2px solid #374151", marginBottom: "20px" }}>
                ...
                <div style={{ minHeight: "300px" }}>
                    {activeTab === "orders" && (
                        <div>
                            ...
                            {orders.length === 0 ? <p>No orders found.</p> : orders.map((order) => (
                                <div key={order.id} className="order-card" style={{ border: "1px solid #ddd", padding: "15px", borderRadius: "8px", marginBottom: "15px", backgroundColor: "white" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                                        <strong>Order #{order.id}</strong>
                                        <span style={{ color: order.status === "Delivered" ? "#10b981" : order.status === "Processing" ? "#3b82f6" : "#f59e0b", fontWeight: "bold" }}>{order.status}</span>
                                    </div>
                                    ...
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "10px" }}>
                                        ...
                                        <div style={{ display: "flex", gap: "10px" }}>
                                            ...
                                            <button
                                                ...
                                                style={{ backgroundColor: "#3b82f6", color: "white", border: "none", padding: "8px 15px", borderRadius: "5px", cursor: "pointer" }}
                                            >
                                                Track Order
                                            </button>
                                        </div>
                    ...
                    {activeTab === "help" && (
                        <div>
                            ...
                            {faqs.map((faq, idx) => (
                                <div key={idx} className="faq-card" style={{ marginBottom: "20px", backgroundColor: "white", padding: "15px", borderRadius: "8px", border: "1px solid #eee" }}>
                                    <h4 style={{ margin: "0 0 10px 0" }}>❓ {faq.question}</h4>
                                    <p style={{ margin: 0, color: "#9ca3af" }}>{faq.answer}</p>
                                </div>
                            ))}
                            <div style={{ marginTop: "30px", padding: "20px", backgroundColor: "#1f2937", borderRadius: "8px", textAlign: "center" }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: "20px" }}>
                                {coupons.map((coupon, idx) => (
                                    <div key={idx} className="coupon-card" style={{ border: "2px dashed #007bff", padding: "20px", borderRadius: "8px", backgroundColor: "#f8f9fa", position: "relative" }}>
                                        <h2 style={{ color: "#3b82f6", margin: "0 0 10px 0" }}>{coupon.discount}</h2>
                                        <p style={{ fontWeight: "bold", fontSize: "1.2rem", margin: "0 0 5px 0" }}>{coupon.code}</p>
                                        <p style={{ fontSize: "0.8rem", color: "#9ca3af" }}>Expires on: {coupon.expiry}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
